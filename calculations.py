...
class OnusCalculator:
    ...
    def calculate_onus(self, year_base, entity, state, term_num, term_year, rol_uf, df_data):
        ...
        # Get term data
        df_term = self._get_term_data(df_data_count_freq, entity, state, term_num, term_year)
        
        # Get other terms data for comparison
        df_other_terms = self._get_other_terms_data(df_data_count_freq, entity, state, term_num)
    ...
    def _calculate_municipality_factors(self, df_term, df_other_terms, pop_total, rol_uf):
        """Calculate factors and ônus for each municipality"""
        ...
        for mun_code in mun_codes:
            ...
            factor_freq = freq_numerator / freq_denominator if freq_denominator > 0 else 0
    ...
    def validate_calculation_inputs(self, year_base, entity, state, term_num, term_year, rol_uf):
        """
        ...
        if rol_uf <= 0:
            return False, "ROL deve ser maior que zero"
