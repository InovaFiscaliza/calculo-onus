...
class DataProcessor:
    ...
    def get_operators_list(self):
        ...
        return ['ALGAR', 'BRISANET', 'CLARO', 'LIGGA TELECOM', 'LIGUE', 'OI', 
                'SERCOMTEL', 'TIM', 'VIVO', 'WINITY']
    ...
    def get_exclusion_areas(self, year, state, main_service_area):
        ...
        for area in all_areas:
            ...
            if area_mun_codes.issubset(main_area_mun_codes) and area_mun_codes != main_area_mun_codes:
    ...
    def generate_final_df(self, year_base, entity, term_num, term_year, state, 
                         service_area, exclusion_areas, exclusion_municipalities,
                         freq_ini, freq_fin, freq, bandwidth, term_type):
    ...
    def calculate_onus(self, year_base, entity, state, term_num, term_year, rol_uf, df_data):
        ...
        pop_total = df_year_base[['Municipio', 'popMun']].drop_duplicates()['popMun'].sum()
        ...
        for mun_code in mun_codes:
            # Population factor
            pop_factor = df_term[df_term['codMun'] == mun_code]['popMun'].unique() / pop_total
            
            # Frequency factor
            freq_numerator = df_term[df_term['codMun'] == mun_code]['BW_Freq'].unique().sum()
            freq_denominator = freq_numerator + df_other_terms[df_other_terms['codMun'] == mun_code]['BW_Freq'].unique().sum()
